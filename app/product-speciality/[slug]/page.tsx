...
async function fetchProductCategoryData(slug: string) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_PRODUCTS_API_URL}/products?speciality_slug=${slug}`,
      {
        method: "GET",
        headers: {
...
async function fetchCategoryMetaData(slug: string) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_PRODUCTS_API_URL}/seo/get?modal=product_specialities&slug=${slug}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
export async function generateMetadata({
  ...
  try {
    ...
    const canonical = `product-speciality/${slug}`;
