...
const PromotionCard: React.FC<OfferProps> = ({ offer, isExpired = false }) => {
  const [showFull, setShowFull] = useState(false);
  const [maxHeight, setMaxHeight] = useState("4.5rem"); // collapsed height
  const descRef = useRef<HTMLDivElement>(null);

  const imageUrl =
    process.env.NEXT_PUBLIC_PRODUCT_URL + "/" + offer.image?.[0]?.url;
    ...
    >
      <div className="relative">
        ...
        <div className="absolute top-4 right-4">
          {isExpired && (
            <div className="bg-primary2 text-white px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-1">
              Expired
            </div>
          )}
        </div>
